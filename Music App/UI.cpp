...
void UI::run(){
  ...
  while (true){
    ...
    //getting user input
    promptForStr("Music Player (enter cmd or type '.help') "+ UICMD_QUIT, input);

	  if(input.compare(UICMD_QUIT) == 0) 
      break;
      ...
      if((logging == COMMAND) || (logging == BOTH)) 
        logs.push_back(cmd.getCommandString());		  
		  if(cmd.isUICommand()) 
        executeCommand(cmd);
		  else if(cmd.isAppCommand()) 
        owner->executeCommand(cmd);
		  else if(cmd.isDevCommand()) 
  ...
  printMessage("THANK YOU & GOOD BYE!!\n");
