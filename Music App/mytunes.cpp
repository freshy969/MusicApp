...
void MyTunes::executeCommand(Command cmd){
	...
	if(cmd.isCommand(CMD_ADD)) 
		executeCMDADD(cmd);
	else if(cmd.isCommand(CMD_DELETE)) 
		executeCMDDELETE(cmd);
	else if(cmd.isCommand(CMD_SHOW)) 
		executeCMDSHOW(cmd);
	else if(cmd.isCommand(CMD_FOLLOW)) 
		executeCMDFOLLOW(cmd);
}
...
void MyTunes::executeCMDFOLLOW(Command cmd){
	...
	enum arguments {FOLLOW,_U,ID1,_P,PLAYLIST,_F,ID2};
	string userid1= cmd.getToken(ID1);
	string playlist_name=cmd.getToken(PLAYLIST);
	string userid2=cmd.getToken(ID2);
	if(userid2=="stop"){
		User * user1 = users.findByUserID(userid1);
		if(user1 == NULL) 
			return;
		Playlist * student = user1->findPlaylist(playlist_name);
		if (student==NULL) 
			return;
		else{
			vector<Follow*> follow_list=followers.getCollection();
			for (vector<Follow*>::iterator itr =follow_list.begin();itr != follow_list.end(); ++itr)
				(*itr)->removefollower(*student);
		}
	}
	else{
		if(userid1==userid2){
		...
		if(user1 == NULL) 
			return;
		User * user2 = users.findByUserID(userid2);
		if(user2 == NULL) 
			return;
		Playlist * master = user2->findPlaylist(playlist_name);
		if(master == NULL) 
			return;
		Playlist * student = user1->findPlaylist(playlist_name);
		if(student == NULL) 
			return;
		// removing deadlock
		Follow* follow=followers.find(student);
		if(follow!=NULL && follow->findfollower(master)!=NULL){
			return;
		}
		follow=followers.find(master);
		if(follow==NULL){
			follow = new Follow(playlist_name,master);
			followers.add(*follow);
		}
		if(follow->findfollower(student)==NULL){
			cout<<"Adding follower"<<endl;
			follow->addfollower(*student);
		}
		else{
			// multiple adding
		}
	}
	cout<<"EXECUTING FOLLOW Commands "<<endl;
}
...
void MyTunes::executeCMDADD(Command cmd){
    ...
    //add -l gjones "mysongs" 6
	    ...
	    if(cmd.hasToken("-s"))
			...
			executeAddRecording(cmd);	
		else if(cmd.hasToken("-t")) 
			executeAddTrack(cmd);	
		else if(cmd.hasToken("-u")) 
			executeAddUser(cmd);	
		else if(cmd.hasToken("-p")) 
			executeAddPlaylist(cmd);	
		else if(cmd.hasToken("-l")) 
			executeAddPlaylistTrack(cmd);	
		else 
			view.printOutput("EXECUTING: " + cmd.getCommandString());
}
...
void MyTunes::executeAddSong(Command cmd){
	...
	if(!cmd.isValidIndex(cmd.getToken(ID))) 
		return;
	int id = -1;
	if(cmd.isTokenAt(ID,"?")) 
		id = nextAvailableSongID++;
    else 
    	id = stoi(cmd.getToken(ID));
	
	if(nextAvailableSongID <= id) 
		nextAvailableSongID = id + 1;
...
void MyTunes::executeAddRecording(Command cmd){
	...
	int id = -1;
	if(cmd.isTokenAt(ID,"?")) 
		id = nextAvailableRecordingID++;
    else 
    	id = stoi(cmd.getToken(ID));
	
	if(nextAvailableRecordingID <= id) 
		nextAvailableRecordingID = id + 1;
...
void MyTunes::executeAddTrack(Command cmd){
	...
	if(!cmd.isValidIndex(cmd.getToken(ID))) 
		return;
	if(!cmd.isValidIndex(cmd.getToken(SONG_ID))) 
		return;
	if(!cmd.isValidIndex(cmd.getToken(TRACK_NUMBER))) 
		return;

	int id = -1;
	if(cmd.isTokenAt(ID,"?")) 
		id = nextAvailableTrackID++;
    else 
    	id = stoi(cmd.getToken(ID));
	if(nextAvailableTrackID <= id) 
		nextAvailableTrackID = id + 1;
	
	Recording * recording;
	//tracks don't have to be associated with recordings
	if(cmd.getToken(RECORDING_ID).compare("null") == 0) 
		recording = NULL;
	else {
		if(!cmd.isValidIndex(cmd.getToken(RECORDING_ID))) 
			return;
...
void MyTunes::executeAddPlaylistTrack(Command cmd){
	...
	if(!cmd.isValidIndex(cmd.getToken(TRACK_ID))) 
		return;
	...
	Follow* follow=followers.find(playlist);
	if(follow!=NULL){
		follow->addTrack(*track);
	}
...
void MyTunes::executeDeleteUserPlaylistTrack(Command cmd){
    ...
    if(!cmd.isValidIndex(trackIDstring)) 
    	return;
